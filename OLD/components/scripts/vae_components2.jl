...
X = rand(MvNormal([0;5], I), 800);
Y = rand(MvNormal([-5;-3], I), 200);
Z = rand(MvNormal([5;-3], I), 400);
dt = hcat(X, Y, Z);
scatter(dt[1,:],dt[2,:],aspect_ratio=:equal)
ndat = size(dt, 2);
...
scatter(dt[1,:],dt[2,:],zcolor=labels,aspect_ratio=:equal,label="data classes")

gen = generate_new(f,700,α)
gen_labels = create_labels_new(gen)
scatter(dt[1,:],dt[2,:],legend=:topright,label="data",aspect_ratio=:equal);
scatter!(gen[1,:],gen[2,:],label="f(e)")
